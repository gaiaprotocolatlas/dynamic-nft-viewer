import { ArrayUtil, EventHandler, TreeNode } from "gaia-commons-ts";
...
export default class DomNode<EL extends HTMLElement = HTMLElement> extends TreeNode {
    ...
    public onWindow<ET extends Event>(eventName: string, eventHandler: DomEventHandler<ET, this>): void {
    ...
    public hasClass(className: string): boolean { return this.domElement.classList.contains(className); }
    public toggleClass(className: string): void { this.domElement.classList.toggle(className); }
    public clone() { return new DomNode(this.domElement.cloneNode(true) as EL); }
    ...
    public animate(options: {
        ...
        let keyframesStartStyle: Style = {};
        let keyframesFinalStyle: Style = {};
